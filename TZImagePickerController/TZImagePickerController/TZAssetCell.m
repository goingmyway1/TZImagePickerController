...
- (void)applySelectionState:(BOOL)selected animated:(BOOL)animated {
    self.selectImageView.image = selected ? self.photoSelImage : self.photoDefImage;
...
// Return YES if a point (in cell's coordinate) is inside the select button area.
- (BOOL)isPointInsideSelectButton:(CGPoint)pointInCell {
    if (!self.selectPhotoButton) return NO;
    return CGRectContainsPoint(self.selectPhotoButton.frame, pointInCell);
}
