...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, PHPhotoLibraryChangeObserver, UIGestureRecognizerDelegate> {
    ...
    // pan-to-select support
    NSMutableSet *_panSelectedIndexPaths;
    BOOL _panSelectionTargetState;
    UIPanGestureRecognizer *_panGesture;
    NSMutableArray *_panOrderedIndexPaths;
    BOOL _prevInteractivePopGestureEnabled;
    id<UIGestureRecognizerDelegate> _prevInteractivePopGestureDelegate;
    NSMutableArray *_disabledEdgePanGestures;
}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // restore interactive pop gesture enabled state and delegate
    if (self.navigationController && self.navigationController.interactivePopGestureRecognizer) {
        self.navigationController.interactivePopGestureRecognizer.delegate = _prevInteractivePopGestureDelegate ?: self.navigationController.interactivePopGestureRecognizer.delegate;
        self.navigationController.interactivePopGestureRecognizer.enabled = _prevInteractivePopGestureEnabled;
    }
    // restore any edge pan gestures we disabled
    if (_disabledEdgePanGestures.count) {
        for (NSDictionary *info in _disabledEdgePanGestures) {
            UIScreenEdgePanGestureRecognizer *g = info[@"gesture"];
            NSNumber *enabled = info[@"enabled"];
            id delegate = info[@"delegate"];
            g.enabled = enabled.boolValue;
            if (delegate != [NSNull null]) g.delegate = delegate;
        }
        _disabledEdgePanGestures = nil;
    }
}
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        // add pan gesture recognizer for quick swipe-to-select like system Photos
        if (!_panGesture) {
            _panSelectedIndexPaths = [NSMutableSet set];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Disable interactive pop gesture (left-edge swipe) to avoid conflict with pan-to-select.
    // Save and replace delegate so we can fully prevent pop while this VC is visible.
    if (self.navigationController && self.navigationController.interactivePopGestureRecognizer) {
        _prevInteractivePopGestureEnabled = self.navigationController.interactivePopGestureRecognizer.enabled;
        _prevInteractivePopGestureDelegate = self.navigationController.interactivePopGestureRecognizer.delegate;
        // set delegate to self and disable; gestureRecognizerShouldBegin will also block it.
        self.navigationController.interactivePopGestureRecognizer.delegate = (id<UIGestureRecognizerDelegate>)self;
        self.navigationController.interactivePopGestureRecognizer.enabled = NO;
    }
    // Additionally disable any left-edge UIScreenEdgePanGestureRecognizer installed on the nav controller's view (some libs add full-screen or edge gestures).
    _disabledEdgePanGestures = [NSMutableArray array];
    for (UIGestureRecognizer *g in self.navigationController.view.gestureRecognizers) {
        if ([g isKindOfClass:[UIScreenEdgePanGestureRecognizer class]]) {
            UIScreenEdgePanGestureRecognizer *edge = (UIScreenEdgePanGestureRecognizer *)g;
            if (edge.edges & UIRectEdgeLeft) {
                NSDictionary *info = @{@"gesture": edge, @"enabled": @(edge.enabled), @"delegate": edge.delegate ?: [NSNull null]};
                [_disabledEdgePanGestures addObject:info];
                edge.enabled = NO;
            }
        }
    }
...
#pragma mark - Pan-to-select Gesture

- (void)handlePanGesture:(UIPanGestureRecognizer *)pan {
    CGPoint location = [pan locationInView:_collectionView];
    NSIndexPath *indexPath = [_collectionView indexPathForItemAtPoint:location];
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;

    if (pan.state == UIGestureRecognizerStateBegan) {
        [_panSelectedIndexPaths removeAllObjects];
        _panOrderedIndexPaths = [NSMutableArray array];
        if (!indexPath) return;
        if (indexPath.item == [self getAddMorePhotoCellIndex] || indexPath.item == [self getTakePhotoCellIndex]) return;
        UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:indexPath];
        if (![cell isKindOfClass:[TZAssetCell class]]) return;
        TZAssetCell *assetCell = (TZAssetCell *)cell;
        // begin pan selection when user started on the cell (image or select button)
        CGPoint pointInCell = [pan locationInView:assetCell];
        if (!CGRectContainsPoint(assetCell.bounds, pointInCell)) return;
        NSInteger index = indexPath.item;
        if (!tzImagePickerVc.sortAscendingByModificationDate) {
            index -= [self getAllCellCount] - _models.count;
        }
        ...
        TZAssetModel *model = _models[index];
        // if starting on a selected model, we will deselect items we pass; otherwise select them
        _panSelectionTargetState = !model.isSelected;
        [self processPanIndexPath:indexPath];
    } else if (pan.state == UIGestureRecognizerStateChanged) {
        if (!indexPath) return;
        // if we've already processed this indexPath
        if ([_panSelectedIndexPaths containsObject:indexPath]) {
            // check if user moved back over previous items and undo later items
            NSUInteger pos = [_panOrderedIndexPaths indexOfObject:indexPath];
            if (pos != NSNotFound && pos < _panOrderedIndexPaths.count - 1) {
                // undo items after pos
                NSArray *toUndo = [_panOrderedIndexPaths subarrayWithRange:NSMakeRange(pos+1, _panOrderedIndexPaths.count - pos - 1)];
                for (NSIndexPath *ip in toUndo) {
                    UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:ip];
                    if ([cell isKindOfClass:[TZAssetCell class]]) {
                        TZAssetCell *assetCell = (TZAssetCell *)cell;
                        if (assetCell.didSelectPhotoBlock) {
                            assetCell.didSelectPhotoBlock(assetCell.selectPhotoButton.isSelected);
                        }
                    }
                    [_panSelectedIndexPaths removeObject:ip];
                }
                // trim ordered array
                [_panOrderedIndexPaths removeObjectsInRange:NSMakeRange(pos+1, _panOrderedIndexPaths.count - pos - 1)];
            }
            return;
        }
        [self processPanIndexPath:indexPath];
...
- (void)processPanIndexPath:(NSIndexPath *)indexPath {
    if (!indexPath) return;
    if (indexPath.item == [self getAddMorePhotoCellIndex] || indexPath.item == [self getTakePhotoCellIndex]) return;
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    NSInteger index = indexPath.item;
    ...
    if (_panSelectionTargetState && model.isSelected) return;
    if (!_panSelectionTargetState && !model.isSelected) return;
    UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:indexPath];
    if (![cell isKindOfClass:[TZAssetCell class]]) return;
    TZAssetCell *assetCell = (TZAssetCell *)cell;
    if (assetCell.selectPhotoButton.hidden) return;
    if (assetCell.didSelectPhotoBlock) {
        // pass current selection state so the existing block toggles appropriately
        assetCell.didSelectPhotoBlock(assetCell.selectPhotoButton.isSelected);
        [_panSelectedIndexPaths addObject:indexPath];
        [_panOrderedIndexPaths addObject:indexPath];
        // Mirror the UI updates done by selectPhotoButtonClick: so pan selection shows same highlight/animation
        if ([assetCell respondsToSelector:@selector(applySelectionState:animated:)]) {
            [assetCell applySelectionState:assetCell.selectPhotoButton.isSelected animated:YES];
        }
        if (assetCell.selectPhotoButton.isSelected) {
            [assetCell requestBigImage];
        } else {
            [assetCell cancelBigImageRequest];
        }
    }
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer == _panGesture) {
        CGPoint location = [_panGesture locationInView:_collectionView];
        NSIndexPath *indexPath = [_collectionView indexPathForItemAtPoint:location];
        if (!indexPath) return NO;
        UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:indexPath];
        if (![cell isKindOfClass:[TZAssetCell class]]) return NO;
        TZAssetCell *assetCell = (TZAssetCell *)cell;
        CGPoint pointInCell = [_panGesture locationInView:assetCell];
        // start when touching anywhere on the cell (image or select button) to support swiping across images
        if (!CGRectContainsPoint(assetCell.bounds, pointInCell)) return NO;
        // avoid interfering with interactivePopGestureRecognizer near left edge
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    // allow simultaneous with collection view scrolling to avoid blocking scroll
    if (otherGestureRecognizer == _collectionView.panGestureRecognizer) return YES;
    // but don't recognize simultaneously with interactive pop
    if (otherGestureRecognizer == self.navigationController.interactivePopGestureRecognizer) return NO;
    return NO;
}

- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldBeRequiredToFailByGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    // Let the navigation controller's interactive pop gesture take precedence.
    if (gestureRecognizer == _panGesture && otherGestureRecognizer == self.navigationController.interactivePopGestureRecognizer) {
        return YES;
    }
    return NO;
}
